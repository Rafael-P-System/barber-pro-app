...
export default function AdminDashboard() {
  ...
  useEffect(() => { carregarBarbearias(); }, []);

  const carregarBarbearias = async () => {
    const response = await api.get('/admin/barbearias'); // Rota que criaremos no Java
  ...
  const toggleStatus = async (id, statusAtual) => {
    ...
    carregarBarbearias(); // Atualiza a lista
  };
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', padding: 15, borderRadius: 10, flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10, elevation: 3 },
